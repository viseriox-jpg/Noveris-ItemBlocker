...
public final class RestrictionCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> d) {
        ...
        root.then(Commands.literal("list").executes(c -> { RestrictionData data = RestrictionManager.data(c.getSource().getLevel()); c.getSource().sendSuccess(() -> Component.literal("[Noveris 1.0.6] Regras de itens: " + data.itemRules.size()), false); if (data.itemRules.isEmpty()) c.getSource().sendSuccess(() -> Component.literal("Nenhuma regra de item cadastrada."), false); else data.itemRules.forEach((id, type) -> c.getSource().sendSuccess(() -> Component.literal(id + " - " + (type == RestrictionType.BLOCKED ? "BLOQUEADO" : "JOGADORES ESPECIFICOS")), false)); return data.itemRules.size(); }));
        d.register(root);
    }
