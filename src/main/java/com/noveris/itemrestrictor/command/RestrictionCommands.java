...
public final class RestrictionCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> d) {
        d.register(Commands.literal("itemrestrict").requires(s -> s.hasPermission(2))
            .then(Commands.literal("open").executes(c -> { ServerPlayer p = c.getSource().getPlayerOrException(); NetworkHandler.sendOpen(p); return 1; }))
            .then(Commands.literal("item").then(Commands.literal("block").then(Commands.argument("item", StringArgumentType.word()).suggests((c,b) -> SharedSuggestionProvider.suggestResource(BuiltInRegistries.ITEM.keySet(), b)).executes(c -> item(c.getSource(), StringArgumentType.getString(c,"item"), RestrictionType.BLOCKED))))
            .then(Commands.literal("allow").then(Commands.argument("item", StringArgumentType.word()).then(Commands.argument("player", StringArgumentType.word()).executes(c -> allow(c.getSource(), StringArgumentType.getString(c,"item"), StringArgumentType.getString(c,"player"))))))
            .then(Commands.literal("mod").then(Commands.literal("block").then(Commands.argument("modid", StringArgumentType.word()).executes(c -> mod(c.getSource(), StringArgumentType.getString(c,"modid"), true)))).then(Commands.literal("unblock").then(Commands.argument("modid", StringArgumentType.word()).executes(c -> mod(c.getSource(), StringArgumentType.getString(c,"modid"), false)))))
            .then(Commands.literal("list").executes(c -> { c.getSource().sendSuccess(() -> Component.literal("Items: " + RestrictionManager.data(c.getSource().getLevel()).itemRules.size()), false); return 1; })));
    }
